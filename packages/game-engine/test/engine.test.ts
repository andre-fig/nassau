...
import { applyAction, createGame, getPlayerView, getSalePreview, GOODS, GOOD_INFO } from '../src/index';

const game = (seed = 11) => createGame({ players: [{ id: 'a', displayName: 'Ana' }, { id: 'b', displayName: 'Bia' }] }, seed);
...
describe('Nassau game engine', () => {
  it('creates the deterministic setup with 43 items and a five-item port', () => {
    ...
    expect(state.deck.length + state.port.length + state.players.reduce((sum, p) => sum + p.goods.length + p.crew, 0)).toBe(43);
  ...
  it('takes goods and replenishes the port', () => {
    ...
    const result = applyAction(state, { type: 'take-good', playerId: 'a', itemId: good.id });
    expect(result.state.port).toHaveLength(5);
    expect(result.state.players[0].goods.some((item) => item.id === good.id)).toBe(true);
  });

  it('rejects a one-for-one trade and crew as a received item', () => {
    const state = game();
    const goods = state.port.filter((item) => item.type !== 'crew');
    expect(() => applyAction(state, { type: 'trade', playerId: state.currentPlayerId, takeItemIds: [goods[0].id], giveGoodIds: [], giveCrewCount: 1 })).toThrow();
  });

  it('uses highest rewards first and applies only one contract', () => {
    const state = game();
    const player = state.players.find((p) => p.id === state.currentPlayerId)!;
    player.goods = Array.from({ length: 5 }, (_, i) => ({ id: `r-${i}`, type: 'rum' as const }));
    const result = applyAction(state, { type: 'sell', playerId: player.id, goodType: 'rum', quantity: 5 });
    expect(result.event.payload).toMatchObject({ points: 15, contractPrestige: 9 });
    expect(player.prestige).toBe(0);
    expect(result.state.players.find((p) => p.id === player.id)?.prestige).toBe(24);
  });
  ...
  it('enforces the noble minimum and previews a sale', () => {
    ...
    expect(getSalePreview(state, player.id, 'royal-jewels', 1).canSell).toBe(false);
    expect(() => applyAction(state, { type: 'sell', playerId: player.id, goodType: 'royal-jewels', quantity: 1 })).toThrow();
  });

  it('finishes after two value tracks are empty and awards majority crew', () => {
    const state = game();
    const player = state.players.find((p) => p.id === state.currentPlayerId)!;
    player.goods = [{ id: 'r-1', type: 'rum' }, { id: 'r-2', type: 'rum' }, { id: 'r-3', type: 'rum' }, { id: 'r-4', type: 'rum' }, { id: 'r-5', type: 'rum' }];
    state.values.rum = [1];
    ...
    expect(result.state.result?.players.find((p) => p.id === player.id)?.crewBonus).toBe(5);
